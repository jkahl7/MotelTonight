...
@property (nonatomic) BOOL isInTesting;
...
#pragma seedDatabaseIfNeeded // this will check if the dabase is empty & seed if it is.
- (void)seedDatabaseIfNeeded
{
  NSFetchRequest *fetchRequest = [[NSFetchRequest alloc] initWithEntityName:@"Hotel"];
  NSError *initialFetchError;
  
  //returns an NSInteger object with the number of objects returned by the fetch request
  NSInteger results = [self.managedObjectContext countForFetchRequest:fetchRequest
                                                                 error:&initialFetchError];
  if (results == 0) //zero is nil?
  {
    NSURL *seedURL   = [[NSBundle mainBundle] URLForResource:@"seed"
                                               withExtension:@"json"];
    
    NSData *seedData = [[NSData alloc] initWithContentsOfURL:seedURL];
    
    NSError *jsonError;
    NSDictionary *rootDictionary = [NSJSONSerialization JSONObjectWithData:seedData
                                                                   options:0
                                                                     error:&jsonError];
    if (!jsonError)
    {
      ...
      {
        Hotel *hotel = [NSEntityDescription insertNewObjectForEntityForName:@"Hotel"
...
#pragma CoreData functionality //  below is boilerplate code provided to implement CoreData functionality
...
- (NSManagedObjectModel *)managedObjectModel
{
  ...
  NSURL *modelURL     = [[NSBundle mainBundle] URLForResource:@"MotelTonight"
                                                withExtension:@"momd"];
